...
$tw.PDFDestinationKindMapping = {
    "XYZ": $tw.PDFDestinationKind.OffsetAndZoom,
    "Fit": $tw.PDFDestinationKind.EntirePage,
    "FitH": $tw.PDFDestinationKind.VerticalPosition, // Fit is named to the coordinate that is not specified.
    "FitV": $tw.PDFDestinationKind.HorizontalPosition,
};
    
$tw.PDFDestination = function(document, page) {
}

$tw.PDFColorSpace = function(document, root, kind, bitsPerComponent) {
...
$tw.PDFColorSpace.DeviceColorSpaces = [
    $tw.PDFColorSpace.DeviceGrayColorSpace,
    $tw.PDFColorSpace.DeviceRGBColorSpace
];

$tw.PDFColorSpace.ComponentsPerColorSpace =  {
    [$tw.PDFColorSpace.ColorSpaces[$tw.PDFColorSpace.DeviceGrayColorSpace]]: 1,
    [$tw.PDFColorSpace.ColorSpaces[$tw.PDFColorSpace.DeviceRGBColorSpace]]: 3
}

$tw.PDFColorSpace.PNGColorTypeMap = {
    [$tw.PDFColorSpace.ColorSpaces[$tw.PDFColorSpace.DeviceGrayColorSpace]]: 0,
...
$tw.PDFIndexedColorSpace = function(document, root, bitsPerComponent) {
    $tw.PDFColorSpace.call(
        ...
        document,
        root,
        $tw.PDFColorSpace.ColorSpaces[$tw.PDFColorSpace.IndexedColorSpace],
        bitsPerComponent);

    this.read();
}

$tw.PDFIndexedColorSpace.prototype = Object.create($tw.PDFColorSpace.prototype);
$tw.PDFIndexedColorSpace.prototype.constructor = $tw.PDFIndexedColorSpace;

$tw.PDFIndexedColorSpace.prototype.read = function() {
    var base = this.document.reader.queryArrayObject(this.root, 1);
    if ($tw.PDFColorSpace.DeviceColorSpaces.indexOf(base.value) < 0) {
        // HACK.
        return;
       //$tw.util.error("expected base color space to be DeviceN: " + base.value);
    }

    this.base = new $tw.PDFColorSpace(
        this.document.reader,
        base,
        $tw.PDFColorSpace.ColorSpaces[base.value],
        this.bitsPerComponent);
    this.maxIndex = this.document.reader.queryArrayObject(this.root, 2).value;

    // read the table.
    var numberComponents = $tw.PDFColorSpace.ComponentsPerColorSpace[this.base.kind];
...
$tw.PDFForm = function(document, root, objectID) {
    this.document = document;
    this.root = root;
    ...
    var bbox = this.attributes.queryObject($tw.PDFForm.BBoxKey);
...
$tw.PDFForm.BBoxKey = "BBox";
$tw.PDFForm.prototype.read = function() {
    if (this.data != null ) {
        return this.data;
    }
...
$tw.PDFImage = function(document, parent, image) {
    this.document = document;
    this.parent = parent;
    this.image = image;
    ...
    if (colorValue.getType() == pdf.ePDFObjectArray) {
        ...
        if (specialName == $tw.PDFColorSpace.IndexedColorSpace) {
    ...
    else if ($tw.PDFColorSpace.DeviceColorSpaces.indexOf(colorValue.value) >= 0) {
        this.color = new $tw.PDFColorSpace(
            this.document,
            colorValue,
...
$tw.PDFImage.prototype.read = function() {
    ...
    if (this.color instanceof $tw.PDFIndexedColorSpace) {
        var numberComponents = $tw.PDFColorSpace.ComponentsPerColorSpace[this.color.base.kind];
        var indexedData = stream.read(this.width * this.height * numberComponents);
        colorType = $tw.PDFColorSpace.PNGColorTypeMap[this.color.base.kind];
...
$tw.PDFDocumentOutlineItem = function(document, item) {
    this.document = document;
    this.item = item;
}

$tw.PDFDocumentOutlineItem.TitleKey = "Title";

// Optional (mutually exclusive with "Actions");
$tw.PDFDocumentOutlineItem.DestKey = "Dest";    

$tw.PDFDocumentOutlineItem.prototype.read = function() {
    this.title = this.document.reader.queryDictionaryObject(
        this.item, $tw.PDFDocumentOutlineItem.TitleKey);
    // holds named destination.
    this.name = null;
    
    var dest = this.document.reader.queryDictionaryObject(
        this.item, $tw.PDFDocumentOutlineItem.DestKey);
    if (dest !== undefined) {
        this.name = dest.value;
    }
}
    
$tw.PDFDocumentOutline = function(document, outline, level) {
    this.document = document;
...
$tw.PDFDocumentOutline.OutlinesKey = "Outlines";
$tw.PDFDocumentOutline.CountKey = "Count";

$tw.PDFDocumentOutline.prototype.read = function() {
    // sign of values imply "visibility".
    this.count = Math.abs(
        this.document.reader.queryDictionaryObject(
            this.outline, $tw.PDFDocumentOutline.CountKey));
    ...
    for (var index = 0; index < this.count; index++) {
        var currentItem = new $tw.PDFDocumentOutlineItem(this.document, current);
        ...
        var childCount = this.document.reader.queryDictionaryObject(
            current, $tw.PDFDocumentOutline.CountKey);
        if (childCount !== undefined && (Math.abs(childCount) > 0)) {
            var childOutline = new $tw.PDFDocumentOutline(
...
$tw.PDFDocumentCatalog.RootKey = "Root";
    
$tw.PDFDocumentCatalog.prototype.read = function() {
    if (this.outline != null) {
        this.outline.read();
    }
}
    
$tw.PDFDocumentMetadata = function(document) {
    this.document = document;
    this.trailer = this.document.reader.getTrailer();
}

$tw.PDFDocumentMetadata.InfoKey = "Info";
$tw.PDFDocumentMetadata.AuthorKey = "Author";
$tw.PDFDocumentMetadata.KeywordsKey = "Keywords";
$tw.PDFDocumentMetadata.SubjectKey = "Subject";
$tw.PDFDocumentMetadata.TitleKey = "Title";
$tw.PDFDocumentMetadata.CreationDateKey = "CreationDate";
$tw.PDFDocumentMetadata.ModDateKey = "ModDate";
    
$tw.PDFDocumentMetadata.prototype.read = function() {
    this.info = this.document.reader.queryDictionaryObject(
        this.trailer, $tw.PDFDocumentMetadata.InfoKey);

    this.author = this.document.reader.queryDictionaryObject(
        this.info, $tw.PDFDocumentMetadata.AuthorKey);
    if (this.author != undefined) {
        this.author = this.author.value;
    }
    
    this.keywords = this.document.reader.queryDictionaryObject(
        this.info, $tw.PDFDocumentMetadata.KeywordsKey);
    if (this.keywords != undefined) {
        this.keywords = this.keywords.value;
    }
    
    this.subject = this.document.reader.queryDictionaryObject(
        this.info, $tw.PDFDocumentMetadata.SubjectKey);
    if (this.subject != undefined) {
        this.subject = this.subject.value;
    }
    
    this.title = this.document.reader.queryDictionaryObject(
        this.info, $tw.PDFDocumentMetadata.TitleKey);
    if (this.title != undefined) {
        this.title = this.title.value;
    }

    this.created = this.document.reader.queryDictionaryObject(
        this.info, $tw.PDFDocumentMetadata.CreationDateKey);
    if (this.created != undefined) {
        this.created = this.created.value;
    }
    
    this.modified = this.document.reader.queryDictionaryObject(
        this.info, $tw.PDFDocumentMetadata.ModDateKey);
...
$tw.PDFExternalObject = function(document, root) {
    this.document = document;
    this.root = root;
}

$tw.PDFExternalObject.ImageSubtype = "Image";
$tw.PDFExternalObject.FormSubtype = "Form";

// Note: CCITTFaxDecode seems to cause a segmentation fault.
$tw.PDFExternalObject.SupportedFilters = ["DCTDecode", "FlateDecode"]

$tw.PDFExternalObject.prototype.read = function() {
    ...
    for (var nameIndex = 0; nameIndex < this.names.length; nameIndex++) {        
        ...
        if (objectMetadata.Subtype.value == $tw.PDFExternalObject.ImageSubtype) {
            ...
            // an image!
            var image = new $tw.PDFImage(this.document, null, object);
...
$tw.PDFPageResources = function(document, root) {
    this.document = document;
    this.root = root;
    this.xobject = null;

    var xobjectValue = this.document.reader.queryDictionaryObject(
        this.root, $tw.PDFPageResources.XObjectKey);
    if (xobjectValue !== undefined) {
        this.xobject = new $tw.PDFExternalObject(this.document, xobjectValue);
    }
}

$tw.PDFPageResources.XObjectKey = "XObject";

$tw.PDFPageResources.prototype.read = function() {
...
$tw.PDFPage = function(document, index, metadata) {
    this.document = document;
    this.index = index;
    this.metadata = metadata;

    this.resources = new $tw.PDFPageResources(
        this.document,
        this.document.reader.queryDictionaryObject(this.metadata, $tw.PDFPage.ResourcesKey));

    this.hasRead = false;
}

$tw.PDFPage.ResourcesKey = "Resources";
...
$tw.PDFDocument = function(path, debug) {
    ...
    this.metadata = new $tw.PDFDocumentMetadata(this);
    this.metadata.read();

    this.catalog = new $tw.PDFDocumentCatalog(this);
    this.catalog.read();
...
$tw.PDFDocument.prototype.saveAllImages = function(basePath) {
...
$tw.fixupPDFDateStrings = function(value) {
    if (value === undefined) {
        return "";
    }
    ...
    // At some point actually convert these to UTC.
    ...
    if (minusIndex >= 0) {
        value = value.substring(0, minusIndex);
    }

    var isoIndex = value.indexOf("Z");
    if (isoIndex >= 0) {
        value = value.substring(0, isoIndex);
    }

    return value;
}

$tw.generateDocumentViewerText = function(filepath, document) {
    var header = "# Document";
    var link = "[" + (document.metadata.title || "N/A") + "](pdf://" + path.basename(filepath) + ")";

    return [header, link].join("\n");
}

$tw.generateDocumentText = function(filepath, document) {
    // This is pretty interesting. We can try and parse an "outline" if needed.
    var viewer = $tw.generateDocumentViewerText(filepath, document);
    return viewer;
}

$tw.generatePDFDataTiddler = function(document, parentName) {
    var data = {
...
$tw.loadTiddlersFromDocument = function(filepath, debug) {
    ...
    var dataTiddler = $tw.generatePDFDataTiddler(document, title);

    // create an additional "data" tiddler that represents the PDF.
    return {filepath: filepath, type: type, tiddlers: [viewTiddler, dataTiddler], hasMetaFile: false };
};
...
$tw.loadWikiTiddlers = function(wikiPath,options) {
    ...
    var wikiDocumentsPath = path.resolve(wikiPath,$tw.config.wikiDocumentsSubDir);
    if(fs.existsSync(wikiDocumentsPath)) {
        console.log("loading wiki \"documents\"");
        var documents = fs.readdirSync(wikiDocumentsPath);
        $tw.utils.each($tw.loadDocuments(documents, wikiDocumentsPath), function(tiddlerFile) {
            if(!options.readOnly && tiddlerFile.filepath) {
                $tw.utils.each(tiddlerFile.tiddlers,function(tiddler) {
                    console.log("document tiddler incoming:");
                    console.log(tiddler);
                    $tw.boot.files[tiddler.title] = {
                        filepath: tiddlerFile.filepath,
                        type: tiddlerFile.type,
